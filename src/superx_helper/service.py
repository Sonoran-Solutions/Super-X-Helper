...
from superx_helper.collector import get_battery_info, get_display_info, get_storage_inventory
from superx_helper.contracts import CapabilityId, CapabilityRecord, CapabilitySnapshot, CapabilityStatus, SafetyState
    ...
    *,
    ...
    if low.has_cpu_boost:
        records.append(CapabilityRecord(
            CapabilityId.CPU_BOOST.value, "CPU Boost",
            ...
            observed_value=platform.read_cpu_boost(), allowed_values=[False, True],
            owner="Linux cpufreq", backend="platform",
    ...
    if low.has_epp_control:
        records.append(CapabilityRecord(
            CapabilityId.CPU_EPP.value, "Energy Performance Preference",
            CapabilityStatus.CONFIRMED_LOCAL if authorized(CapabilityId.CPU_EPP) else CapabilityStatus.SUPPORTED_UNVERIFIED,
            read_supported=True, write_supported=True,
            locally_validated=authorized(CapabilityId.CPU_EPP), observed_value=platform.read_epp(),
            allowed_values=low.epp_available_preferences, owner="amd-pstate-epp", backend="platform",
    ...
    records.append(CapabilityRecord(
        CapabilityId.POWER_TARGET.value, "Power Target",
        CapabilityStatus.SUPPORTED_UNVERIFIED if low.cpu_power_method != "none" else CapabilityStatus.UNAVAILABLE,
        read_supported=low.cpu_power_method != "none", write_supported=False, locally_validated=False,
        owner="Linux powercap / future validated adapter", backend="platform",
        reason_unavailable=("Power telemetry/interface exists, but no production-qualified Super X power-target write path is selected yet." if low.cpu_power_method != "none" else "No supported power interface detected."),
    ...
    if low.has_fan_control:
        records.append(CapabilityRecord(
            CapabilityId.INTERNAL_FAN.value, "Internal Fan",
            CapabilityStatus.CONFIRMED_LOCAL if fan_authorized else CapabilityStatus.SUPPORTED_UNVERIFIED,
            read_supported=low.has_fan_telemetry, write_supported=True, locally_validated=fan_authorized,
            observed_value=platform.read_fan_duty(), minimum=0, maximum=100, unit="%",
            owner="oxpec / firmware EC", backend="platform", write_authorized=fan_authorized,
            reason_unavailable=None if fan_authorized else "oxpec hwmon path is present, but fan writes have not passed local production validation.",
            safety_state=SafetyState.NORMAL if fan_authorized else SafetyState.WARNING,
        ))
    elif low.oxpec_driver_available:
        records.append(CapabilityRecord(
            CapabilityId.INTERNAL_FAN.value, "Internal Fan", CapabilityStatus.SUPPORTED_UNVERIFIED,
            read_supported=False, write_supported=False, locally_validated=False,
            owner="oxpec / firmware EC", backend="platform",
    ...
    if low.has_backlight_control:
        ...
        records.append(CapabilityRecord(
            CapabilityId.DISPLAY_BRIGHTNESS.value, "Display Brightness",
            CapabilityStatus.CONFIRMED_LOCAL if bright_authorized else CapabilityStatus.SUPPORTED_UNVERIFIED,
            read_supported=True, write_supported=True, locally_validated=bright_authorized,
            observed_value=platform.read_display_brightness_percent(), minimum=0, maximum=100, unit="%",
            owner="amdgpu backlight", backend="platform", write_authorized=bright_authorized,
    ...
    records.append(CapabilityRecord(
        CapabilityId.DISPLAY_MODE.value, "Display Mode",
        CapabilityStatus.READ_ONLY if connectors else CapabilityStatus.UNAVAILABLE,
        read_supported=bool(connectors), write_supported=False, locally_validated=bool(connectors),
        observed_value=connectors, owner="DRM/compositor", backend="telemetry",
    ...
    records.append(CapabilityRecord(
        CapabilityId.BATTERY_TELEMETRY.value, "Battery",
        CapabilityStatus.READ_ONLY if battery_present else CapabilityStatus.UNAVAILABLE,
        read_supported=battery_present, write_supported=False, locally_validated=battery_present,
        observed_value=battery if battery_present else None, owner="power_supply", backend="telemetry",
    ...
    records.append(CapabilityRecord(
        CapabilityId.BATTERY_CHARGE_CONTROL.value, "Charge Limit / Bypass", CapabilityStatus.UNAVAILABLE,
        read_supported=False, write_supported=False, locally_validated=False, owner="unknown", backend="platform",
    ...
    records.append(CapabilityRecord(
        CapabilityId.MINI_SSD_PRESENCE.value, "Mini SSD Presence", CapabilityStatus.READ_ONLY,
        read_supported=True, write_supported=False, locally_validated=True, observed_value=mini,
        owner="PCIe / Linux NVMe", backend="storage",
    ...
    records.append(CapabilityRecord(
        CapabilityId.MINI_SSD_RELIABILITY.value, "Mini SSD Reliability", CapabilityStatus.READ_ONLY,
        read_supported=True, write_supported=False, locally_validated=True, observed_value="NOT_QUALIFIED",
        owner="Super X Helper research", backend="storage",
    ...
    for capability_id, label in ((CapabilityId.FROST_BAY_TELEMETRY, "Frost Bay Telemetry"), (CapabilityId.FROST_BAY_CONTROL, "Frost Bay Control")):
        records.append(CapabilityRecord(
            capability_id.value, label, CapabilityStatus.RESEARCH_PENDING,
            read_supported=False, write_supported=False, locally_validated=False,
            owner="Super X Helper / BlueZ", backend="frost_bay",
    ...
    records.append(CapabilityRecord(
        CapabilityId.RGB_CONTROL.value, "RGB", CapabilityStatus.UNAVAILABLE,
        read_supported=False, write_supported=False, locally_validated=False, owner="unresolved", backend="device",
        reason_unavailable="No maintained or locally validated Linux RGB control path has been selected yet.",
        safety_state=SafetyState.UNKNOWN,
    ))
    records.append(CapabilityRecord(
        CapabilityId.CONTROLLER_INTEGRATION.value, "Controller / Gyro Integration", CapabilityStatus.SUPPORTED_UNVERIFIED,
        read_supported=False, write_supported=False, locally_validated=False,
        owner="HHD / InputPlumber / Steam", backend="external_integration",
    ...
    records.append(CapabilityRecord(
        CapabilityId.DIAGNOSTICS_EXPORT.value, "Diagnostics Export", CapabilityStatus.CONFIRMED_LOCAL,
        read_supported=True, write_supported=False, locally_validated=True,
        owner="Super X Helper", backend="diagnostics", safety_state=SafetyState.NORMAL,
    ))
    return records
...
def _unavailable(capability_id: CapabilityId, label: str, reason: str) -> CapabilityRecord:
    return CapabilityRecord(
        capability_id.value, label, CapabilityStatus.UNAVAILABLE,
        read_supported=False, write_supported=False, locally_validated=False,
        reason_unavailable=reason, safety_state=SafetyState.UNKNOWN,
    )
